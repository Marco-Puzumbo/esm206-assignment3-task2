...
# start with star warsdata set
sw_split_name <- starwars %>% 
  
  # split the name column
  separate(col = name,
           
           # define two columns it will be split into
...
# start from star wars data set
sw_skin_sep <- starwars %>% 
  
  # call separate and tell it to separate the skin_color column
  separate(col = skin_color, 
           # specify skin_1 and skin_2 as the two new column names
           into = c("skin_1", "skin_2"),
           # define comma as the separating character
           sep = ',',
           # merge everything after the first , into the second column
           extra = "merge",
...
```{r}
# Your code here: 
# start with mpg data frame
mpg_model_class <- mpg %>% 
  # use unite to combine model and class into model_class
  unite(col = model_class,
        # define columns to be combined
        c(model, class),
        # separate with a -
        sep = "-",
        remove = FALSE
...
```{r}
# Your code here: 
# create vector of stored dates from above
some_dates <- c(11061972, 02142007,01301843)

# convert to dates stored as some_dates_converted
...
# start with lakers dat set
gasol_hookshots <- lakers %>% 
  
  # filter for rows with Pau Gasol as the player and type as hook
  filter(player == "Pau Gasol" & type == "hook") %>% 
  
  # update date column to year month date
  mutate(date_new = ymd(date)) %>% 
  
  # create new column of month
  mutate(month_new = month(date_new)) %>% 

  # count up number of entries per month
  count(month_new)
...
# start with storms dataset
storms_danger <- storms %>% 
  # add column danger which populates high for status hurricane and moderate for any other status
